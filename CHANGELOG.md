...
### 🎨 模块30：门户 UI 体验与布局交互优化
- [优化] 门户首页完成控制台风格 UI 刷新，系统列表、统计卡片、管理员工具条和红心支持入口更清晰，重点信息更靠前。
- [优化] 系统卡片操作区改为内容流内排列，避免收藏、复制、编辑、删除等按钮压住标题。
- [修复] 修复页面容器、弹窗和卡片操作按钮导致的横向滚动与元素重叠问题。
- [修复] 修复登录规则中心仍沿用窄弹窗宽度，导致规则编辑区被挤成窄侧栏的问题。
- [优化] 首页只保留默认主题路径，减少主题切换带来的样式冲突和回归风险。
- [优化] Docker / 插件 / 安装文档版本同步提升到 `3.68`。

### 🧪 验证
- [通过] 已执行 `node --check /tmp/sso_portal_inline.js`、`python3 -m unittest tests.test_frontend_regressions -q`、`docker compose config --quiet`，并完成 `6680` Docker 部署健康检查。
